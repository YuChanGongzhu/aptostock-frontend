...
import { usePriceHistory } from "@/state/usePriceHistory";
...
export function AptoStockTabs() {
  ...
  if (!mounted) {
    return (
      <div className="w-full max-w-6xl mx-auto mt-6 flex items-start gap-6">
        {/* Left placeholder (no chart before mount) */}
  ...
  return (
    <div className="w-full max-w-6xl mx-auto mt-6 flex items-start gap-6">
      {/* Left column: K-line panel (toggle) */}
      {chartSym ? (
        <div className="hidden md:block w-[560px]">
          <Card className="glass-card border">
            <CardHeader>
              <CardTitle className="text-lg flex items-center justify-between">
                <span className="gradient-text">{chartSym} K线图（最近约 60 根）</span>
                <Button size="sm" variant="secondary" onClick={() => setChartSym(null)}>隐藏</Button>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <KLineChart candles={candles[chartSym]} width={520} height={300} />
            </CardContent>
          </Card>
        </div>
      ) : (
        ...
        <Card className="glass-card border">
          ...
          <CardContent>
            <div className={`w-full grid grid-cols-3 gap-4 text-sm`}>
              <div
                className={`p-3 rounded-md bg-card/40 border cursor-pointer hover:shadow-lg transition ${chartSym === "TLSA" ? "ring-1 ring-cyan-400/60" : ""}`}
                onClick={() => mounted && pricesReady && toggleChart("TLSA")}
              >
              ...
              <div
                className={`p-3 rounded-md bg-card/40 border cursor-pointer hover:shadow-lg transition ${chartSym === "CRCL" ? "ring-1 ring-cyan-400/60" : ""}`}
                onClick={() => mounted && pricesReady && toggleChart("CRCL")}
              >
